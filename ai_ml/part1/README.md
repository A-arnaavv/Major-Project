...
The default resume path is configured centrally in `ai_ml/part1/paths.py`.

By default, the application looks for:

    data/raw/resume.pdf

To use a different local filename, set `INTERVIEWGPT_RESUME_FILENAME=your_resume.pdf` in your local `.env` file.
